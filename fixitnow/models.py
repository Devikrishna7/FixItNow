...
#     user = models.OneToOneField(CustomUser, on_delete=models.CASCADE, limit_choices_to={'role': 'worker'})
#     phone_number = models.CharField(max_length=15, blank=True, null=True)
#     specialization = models.CharField(max_length=50, choices=CATEGORY_CHOICES, default='Other')  # 🔹 Added
#     status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='Available')

#     def __str__(self):
#         return f"{self.user.username} - {self.specialization} ({self.status})"
...
#     # - If assigned_worker exists and status != Resolved, set worker Busy.
#     # - If status == Resolved, mark assigned worker Available.
#     # - Send email to student when complaint is resolved.
#     # """
#         old_assigned = None
#         old_status = None
...
#             send_mail(
#                 'Complaint Resolved',
#                 f'Hello {self.user.username},\n\nYour complaint "{self.title}" has been resolved.\n\nThank you,\nFixItNow Admin Team',
#                 'admin@fixitnow.com',
#                 [self.user.email],
#                 fail_silently=True,
#             )
...
#     """
#     Automatically create corresponding profile on user creation.
#     """
#     if created:
...
class WorkerProfile(models.Model):
    ...
    user = models.OneToOneField(CustomUser, on_delete=models.CASCADE, limit_choices_to={'role': 'worker'})
    phone_number = models.CharField(max_length=15, blank=True, null=True)
    specialization = models.CharField(max_length=50, choices=CATEGORY_CHOICES, default='Other')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='Available')
...
class Complaint(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        - Free old worker if reassigned
        - Update new worker’s status
        - Send email notification if complaint resolved
        """
        old_assigned = None
        old_status = None
        ...
        if self.assigned_worker:
            try:
                wp_new = WorkerProfile.objects.get(user=self.assigned_worker)
                wp_new.status = 'Available' if self.status == 'Resolved' else 'Busy'
                wp_new.save()
            except WorkerProfile.DoesNotExist:
                pass
